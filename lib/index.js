...
export default class Rucksack extends RucksackLite {
    ...
    constructor(options = {}) {
        ...
        options.config = options.config || (async (existingConfig) => existingConfig)
        super(options)
    }
    ...
    async bundleJS() {
        let buildOptions = {
            ...
            build: {
                ...
                rollupOptions: {
                    ...
                    onwarn(warning, warn) {
                        if (warning.code === 'MODULE_LEVEL_DIRECTIVE' || warning.code === "EMPTY_IMPORT_META" || warning.code === "PLUGIN_TIMINGS") {
                            return
                        }
        ...
        if (this.options.watch) {
            ...
            buildOptions = await this.options.config(buildOptions)
